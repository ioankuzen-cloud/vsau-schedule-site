...
function renderFilters() {
  ...
  if (state.filters.section && !sections.includes(state.filters.section)) {
    state.filters.section = sections.includes("Основное расписание") ? "Основное расписание" : (sections[0] || "");
  }
...
function filteredLessons() {
  ...
  return state.data.lessons.filter((lesson) => {
    ...
    if (state.selected && !lesson.rooms.includes(state.selected)) return false;
...
function entityCounts() {
  ...
  source.forEach((lesson) => {
    ...
    lesson.rooms.forEach((key) => counts.set(key, (counts.get(key) || 0) + 1));
  });
...
function renderEntities() {
  ...
  const query = state.query.toLowerCase();
  const entities = state.data.rooms
...
function renderSchedule() {
  if (!state.data) {
    els.scheduleTitle.textContent = "Выберите аудиторию";
    els.scheduleContent.innerHTML = `<div class="empty-state">После сборки слева появится список аудиторий.</div>`;
    return;
  }

  els.scheduleTitle.textContent = state.selected || "Выберите аудиторию";
  els.scheduleMeta.textContent = "Расписание аудитории";

  if (!state.selected) {
    els.scheduleContent.innerHTML = `<div class="empty-state">Выберите аудиторию в списке слева.</div>`;
    return;
  }
